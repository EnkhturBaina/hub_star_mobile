	...
	return (
		<ScrollView contentContainerStyle={styles.scrollContainer} showsVerticalScrollIndicator={false} bounces={false}>
			{newsData == null ? (
				...
				<>
					<Text style={{ fontWeight: "bold", fontSize: 16, marginBottom: 10 }}>{newsData.title}</Text>
					<Image
						source={{
						...
						resizeMode="cover"
					/>
					<Text style={{ fontWeight: "bold", fontSize: 16, marginVertical: 10 }}>{newsData.description}</Text>
					<Text style={{ fontSize: 16, marginBottom: 10 }}>{newsData.body}</Text>
				</>
			)}
		</ScrollView>
	);
